...
import commons.util.BinaryFile;
import commons.util.Print;
import commons.util.ProcessFiles;

/**
 * Displays @Unit annotations in compiled class files. If first argument is
 * "-r", @Unit annotations are removed. <br>
 * {Args: ..} <br>
 * {Requires: javassist.bytecode.ClassFile; You must install the Javassist
 * library from http://sourceforge.net/projects/jboss/ }
 */
		...
		try {
			...
			for (CtMethod method : ctClass.getDeclaredMethods()) {
				...
				for (Annotation ann : attr.getAnnotations()) {
					if (ann.getTypeName().startsWith("commons.atunit")) {
						Print.print(ctClass.getName() + " Method: " + mi.getName() + " " + ann);
						if (remove) {
